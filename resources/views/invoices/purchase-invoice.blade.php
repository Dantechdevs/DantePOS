            ...
            <div class="invoice-container">
                <div class="invoice-header">
                    ...
                    <div class="invoice-date">
                        <span>Invoice Date:</span> {{ date('d/m/Y | h:i A', strtotime($purchaseInvoice['date'])) }}
                    </div>
                </div>
                ...
                <div class="invoice-info">
                    ...
                    <div class="invoice-col">
                        ...
                        <address>
                            {{ $purchaseInvoice['supplier']['name'] }}<br>
                            Address: {{ $purchaseInvoice['supplier']['address'] ?? 'N/A' }}<br>
                            Phone: {{ $purchaseInvoice['supplier']['mobile'] ?? 'N/A' }}
                        </address>
                    </div>
                    <div class="invoice-col">
                        <strong>Invoice #:</strong> {{ $purchaseInvoice['purchase_no'] }}<br>
                        ...
                        <?php $previous_balance = $supplierBalance - ($purchaseInvoice['status'] === 'received' ? $purchaseInvoice['grand_total'] : 0); ?>
                    </div>
                </div>
                    ...
                    <table class="table table-striped table-sm invoice-table">
                        ...
                        <tbody>
                            ...
                            @foreach ($purchaseitmesAddons as $addons)
                                ...
                                $counter = $counter + 1;
                                $product = \App\Models\Product::select('id', 'product_code')->find($addons['product_id']);
                                ?>
                                <tr>
                                    <td width="6%">{{ $counter }}</td>
                                    <td width="8%">{{ $addons['productName'] }}</td>
                                    <td width="7%" class="text-center">{{ $addons['price'] }}
                                    </td>
                ...
                <div class="no-print">
                    <a id="printInvoiceButton"
                        href="{{ route('print.purchase.invoice', [$purchaseInvoice['id'], 'logo' => '']) }}"
    ...
    <script>
        ...
        document.querySelectorAll('.logo-wrapper').forEach(wrapper => {
            wrapper.addEventListener('click', function() {
                ...
                const currentHref =
                    "{{ route('print.purchase.invoice', [$purchaseInvoice['id'], 'logo' => '__LOGO__']) }}";
        ...
        if (preselectedElement) {
            ...
            const currentHref = "{{ route('print.purchase.invoice', [$purchaseInvoice['id'], 'logo' => '__LOGO__']) }}";
